...
- **Revised at**: 2026-06-30 — el operador decidió que el catálogo es demasiado delgado para investigadores que cruzan datos; se añade **Track B** (expansión por valor de cruce, co-primario). Ver §7B y registro §9.4. 2026-06-30 — Fase 4 completada. 2026-06-30 — B2.1 completada (CEAD candidate). 2026-06-30 — B2.2 research cerrado (electoral: autoridades_electas viable, resultados_electorales diferido). Plan completo en su alcance definido. Implementación de autoridades_electas y partidos_politicos se deriva a Plan 023.
- **Status**: ✅ Track A + Track B completos (2026-06-30)
...
| 2026-06-30 | Fase 2 (gate de salida) | Gate Fase 2 completado: 2.1 sección README "Por qué confiar" (6 pilares anclados a artefactos), 2.2 case study en `docs/case-study-construccion-chile-hub.md` (~1500 palabras, 5 secciones), 2.3 analítica GoatCounter cookieless (CSP, script, privacy.html, eventos de descarga en app.js) | Mantener: orden 3→4. Fase 3 lista para iniciar. Ola B1 (CASEN+CNE) también lista. | n/a | Claude |
| 2026-06-30 | Ola B1 (gate de salida) | Gate Ola B1 completado: B1.1 `pobreza_comunal` (CASEN — extractor 2 XLSX, contrato, validación, 12 tests), B1.2 `consumo_electrico_comunal` (CNE — extractor Excel, contrato, validación, 9 tests, enriquecimiento CUT desde DPA). Ambos en bundle (`ready`), con source_registry, catalog_config, fichas docs/datasets/ y Makefile. 17 datasets en catálogo, 17 fuentes en registry. | Mantener: Track A Fase 3 (scraping) y Ola B2 (CEAD+electoral) son los próximos pasos. Ola B2 requiere Fase 3.3 (workflow mensual). | Catálogo crece de 15→17 capas (ambas `stable_publishable`); cobertura de dominios añade socioeconómico y energía | Claude |
| 2026-06-30 | Fase 4 — 4.1 (completada) | Tarea 4.1 completada: release notes narrativas en CHANGELOG.md para v1.15.0, v1.16.0, v1.17.0 + proceso documentado en docs/release.md | Mantener: siguiente es 4.2 (post técnico). Sin cambio de jerarquía. | n/a | Claude |
| 2026-06-30 | Fase 4 — 4.2 (completada) | Tarea 4.2 completada: borrador de post técnico para LinkedIn en `docs/linkedin-post-caso-de-estudio.md`, basado en el case study (~3,200 caracteres, 5 decisiones de arquitectura + aprendizajes). El operador decide cuándo publicar. | Mantener: siguiente es 4.3 (gate playground). Sin cambio de jerarquía. | n/a | Claude |
| 2026-06-30 | Ola B2.2 (research cerrado) | B2.2 research completado: documento `docs/legal/b2-2-electoral-research.md`. 6 fuentes evaluadas (TRICEL, SERVEL, Cámara, Senado, BCN, Wikidata). `autoridades_electas` viable (XML Cámara + Wikidata SPARQL, S-M, LOW), `partidos_politicos` viable (S, LOW), `resultados_electorales` diferido hasta fuente estructurada por comuna. Línea roja Ley 19.628 validada. | **Mantener**: plan completo en su alcance. Implementación de autoridades_electas + partidos_politicos se deriva a Plan 023 (follow-up). | Investigación confirma que los agregados electorales son redistribuibles y legalmente sólidos; la limitación es técnica (fuentes), no legal. | Claude |
| 2026-06-30 | Ola B2.1 (completada) | B2.1 completada: extractor `cead_delincuencia_live_extractor.py` (POST al endpoint PHP de CEAD, parseo HTML, 346 comunas, 8 familias de delito × 12 meses), contrato `contracts/datasets/delincuencia_comunal.schema.json`, ficha `docs/datasets/delincuencia_comunal.md`, 18 fuentes en registry, 18 datasets en catalog_config, workflow mensual extendido con job `scrape-cead`. Carril CANDIDATE — fuera del bundle, `review_by: 2026-09-21`. | Mantener: siguiente es B2.2 (electoral research). Sin cambio de jerarquía. | Catálogo: 18 datasets, 18 fuentes. Cobertura de dominios: añade seguridad (delincuencia). | Claude |
| 2026-06-30 | Fase 4 — 4.3 (gate playground) | Gate 4.3 decidido: **NO-GO** — playground DuckDB-Wasm diferido. Evidencia: 36 descargas totales del bundle (27 releases, mediana 1), vistas landing en un dígito/día, GoatCounter < 24h de datos. Decisión documentada en `docs/gate-4-3-decision-playground.md` con 4 condiciones de re-evaluación. Plan 020 queda activo, listo para ejecutar cuando se dispare una condición. | **Redirigir**: la energía de implementación pasa a Ola B2 (B2.1 CEAD candidate + B2.2 electoral research). | n/a | Claude |
| 2026-06-30 | Fase 3 (gate de salida) | Gate Fase 3 completado: 3.1 revisión legal (`docs/legal/fase-3-legal-review.md`, SINIM redistribuible con condiciones, CEAD derive-only, electoral agregados permitidos), 3.2 PoC scraping SINIM exitoso (`sinim_finanzas_live_extractor.py`, cobertura 345/346 municipios, 6 variables financieras, estrategia Playwright+requests), 3.3 workflow mensual aislado (`.github/workflows/monthly-scrape.yml`, desacoplado del build diario, fallback a último snapshot), 3.4 metadata de cadencia honesta (`source_mode=monthly`, campo `cadencia` en source_registry) | Mantener: orden siguiente es Fase 4 (distribución + decisión playground). Ola B2 (CEAD+electoral) puede iniciar ahora que tiene infraestructura de scraping. | `finanzas_municipales` asciende de candidate a `stable_publishable`; 345 municipios con datos live mensuales; workflow de scraping reutilizable por Ola B2 | Claude |
