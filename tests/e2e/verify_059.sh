...
echo "-- make build && publish_hf_dataset.py --dry-run lista 18 parquet --"
make build >/dev/null
DRY_RUN_OUTPUT="$(.venv/bin/python scripts/publish_hf_dataset.py --dry-run)"
echo "$DRY_RUN_OUTPUT"
test "$(echo "$DRY_RUN_OUTPUT" | grep -c 'data/.*\.parquet')" = "18"

echo "-- el dry-run NO lista datasets del carril candidate --"
test "$(echo "$DRY_RUN_OUTPUT" | grep -c 'delincuencia_comunal\|autoridades_locales\|geometria_comunal\|consumo_electrico_comunal')" = "0"
