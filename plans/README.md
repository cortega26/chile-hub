...
| 053 | [Geometría comunal (GeoParquet) + `resolve_by_coords()` reverse geocoding](053-comuna-geometry-and-reverse-geocoding.md) | **P1** | L | MED | — (complementa 050) | 🔶 Steps 0-1 ✓ (2026-07-23) — ADR-011 (estrategia, `proposed`, **requiere ratificación del mantenedor**) y ADR-012 (licencia geometría, `accepted`: fuente `tematico/Comunas_Generalizadas` en `arcgiswebad.bcn.cl`, uso libre con atribución vía `bcn.cl/siit/mapas_vectoriales`) escritos en worktree `worktree-053-comuna-geometry` commit `fcb5f03`. Compuerta de licencia PASA. **Pausado**: Steps 2-5 (extracción + artefacto + `resolve_by_coords()`) esperan ADR-011 `proposed`→`accepted`. |
