...
| :--- | :--- | :--- | :--- | :--- | ---: | :--- | :--- | :--- |
| `regiones` | BCN ArcGIS | `live` | `fresh (821.63h / 2160h)` | `open-attribution (CC BY)` | 16 | `Tier B` | `codigo_region` | `ok` |
| `provincias` | BCN ArcGIS | `live` | `fresh (821.63h / 2160h)` | `open-attribution (CC BY)` | 56 | `Tier B` | `codigo_provincia, codigo_region` | `ok` |
| `comunas` | BCN ArcGIS | `live` | `fresh (821.63h / 2160h)` | `open-attribution (CC BY)` | 346 | `Tier B` | `codigo_comuna, codigo_region` | `ok` |
| `comunas_enriquecidas` | BCN ArcGIS | `live` | `fresh (821.63h / 2160h)` | `open-attribution (CC BY)` | 346 | `Tier B` | `codigo_comuna` | `ok` |
| `indicadores` | Banco Central de Chile (via mindicador.cl) | `live` | `stale (776.97h / 72h)` | `open-attribution (Reproducción libre con citación (BCCh / INE))` | 573 | `Tier A/B` | `fecha, codigo_indicador` | `ok` |
| `censo_comunal` | Instituto Nacional de Estadisticas - Censo 2024 | `live` | `fresh (821.63h / 87600h)` | `open-attribution (CC BY 4.0)` | 346 | `Tier A` | `codigo_comuna, codigo_region` | `ok` |
| `establecimientos_salud` | Ministerio de Salud - Establecimientos de Salud | `live` | `fresh (821.63h / 1080h)` | `open-attribution (CC0)` | 5717 | `Tier A` | `codigo_establecimiento, codigo_comuna` | `ok` |
| `establecimientos_educacionales` | Ministerio de Educación - Directorio Oficial de Establecimientos | `live` | `fresh (821.63h / 8760h)` | `open-attribution (CC-BY-3.0)` | 12898 | `Tier A` | `codigo_comuna` | `ok` |
| `censo_hogares_viviendas` | Instituto Nacional de Estadisticas - Censo 2024 | `live` | `fresh (567.86h / 87600h)` | `open-attribution (CC BY 4.0)` | 346 | `Tier A` | `codigo_comuna, codigo_region` | `ok` |
| `distritos_electorales` | BCN / Biblioteca del Congreso Nacional de Chile | `live` | `fresh (821.63h / 87600h)` | `open-attribution (CC0)` | 346 | `Tier A` | `codigo_comuna` | `ok` |
| `finanzas_municipales` | SINIM - SUBDERE | `monthly` | `fresh (334.27h / 8760h)` | `public-api-review-terms (Datos públicos municipales; términos de reutilización sujetos a revisión)` | 345 | `Tier B` | `anio, codigo_comuna` | `ok` |
| `resultados_educacionales` | Centro de Estudios MINEDUC - Rendimiento 2024 | `live` | `fresh (821.63h / 8760h)` | `open-attribution (CC-BY-3.0)` | 345 | `Tier B` | `anio, codigo_comuna` | `ok` |
| `indicadores_urbanos_siedu` | INE - Sistema de Indicadores y Estándares de Desarrollo Urbano | `live` | `fresh (821.63h / 8760h)` | `open-attribution (Licencia de Datos Abiertos INE)` | 6701 | `Tier B` | `anio, codigo_comuna, codigo_indicador` | `ok` |
...
| `empresas` | Ministerio de Economia, Fomento y Turismo - Registro de Empresas y Sociedades (RES) | `live` | `fresh (821.52h / 1080h)` | `open-attribution (CC-BY)` | 1590979 | `Tier B` | `rut` | `ok` |
| `pobreza_comunal` | Observatorio Social — Ministerio de Desarrollo Social y Familia | `live` | `fresh (821.52h / 175200h)` | `open-attribution (Datos abiertos MDS)` | 690 | `Tier A` | `codigo_comuna, codigo_region` | `ok` |
| `consumo_electrico_comunal` | CNE — Energía Abierta | `fallback` | `fresh (821.52h / 17520h)` | `open-attribution (CC BY)` | 3 | `Tier C` | `codigo_comuna, codigo_region` | `ok` |
| `partidos_politicos` | Cámara de Diputadas y Diputados (datos abiertos) + SERVEL | `live` | `fresh (821.52h / 87600h)` | `open-attribution (CC-BY)` | 36 | `Tier B` | `id_partido` | `ok` |
| `autoridades_electas` | Cámara de Diputadas y Diputados + Senado de Chile | `live` | `fresh (821.5h / 87600h)` | `open-attribution (CC-BY)` | 205 | `Tier B` | `distrito_electoral, circunscripcion_senatorial, codigo_region` | `ok` |
| `estadisticas_vitales` | Instituto Nacional de Estadísticas (INE) — Estadísticas Vitales | `live` | `fresh (0.66h / 8760h)` | `open-attribution (CC BY 4.0)` | 13840 | `Tier A` | `anio, codigo_comuna` | `ok` |
| `permisos_edificacion` | MINVU — Centro de Estudios de Ciudad y Territorio (CEDOC) | `live` | `fresh (0.68h / 1080h)` | `open-attribution (Uso autorizado con cita (MINVU CEDOC))` | 8650 | `Tier A` | `anio, codigo_comuna` | `ok` |
| `calidad_aire` | SINCA — Ministerio del Medio Ambiente | `live` | `fresh (0.17h / 72h)` | `public-api-review-terms (Datos públicos MMA; sin licencia explícita, citar fuente oficial)` | 756 | `Tier B` | `fecha, codigo_comuna, codigo_contaminante` | `ok` |
...
- `warnings`: indicadores live refresh reused last published artifact for missing codes: ipc; indicadores freshness is stale: 776.97h since refresh, policy max is 72h
...
- `freshness`: `fresh (821.63h / 87600h)`
...
- `freshness`: `fresh (567.86h / 87600h)`
- `reuse_policy`: `{"status": "open-attribution", "license": "CC BY 4.0", "license_url": "https://www.ine.gob.cl/terminos-de-uso", "attribution_required": true, "redistribution_ok": true, "summary": "Resultados oficiales del Censo 2024 publicados por el INE."}`
...
- `freshness`: `fresh (821.52h / 175200h)`
...
- `freshness`: `fresh (821.52h / 17520h)`
...
- `freshness`: `fresh (821.52h / 87600h)`
